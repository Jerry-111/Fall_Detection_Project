...
for cID in range(1,25):
    ...
    for camN in range(1,9):
        ...
        while True:
            ...
            adjusted1 = cv2.convertScaleAbs(frame, alpha=alpha, beta=beta)
            adjusted0 = cv2.convertScaleAbs(frame, alpha=1.25, beta=beta)
            adjusted2 = cv2.convertScaleAbs(frame, alpha=2.0, beta=beta)

            cv2.imshow('original', frame)
            cv2.imshow('adjusted0', adjusted0)
            cv2.imshow('adjusted1', adjusted1)
