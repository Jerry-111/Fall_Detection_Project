...
for camN in range(1,9):
    ...
    df = df.append([[False] for _ in range(len(arr))], ignore_index=True)
    ind += len(arr)

    for frame in range(len(arr)):
        if int(arr[frame][5:-4]) - ind in range(rangeList[cID][0], rangeList[cID][1]+1):
            # print(arr[frame])
            df.at[int(arr[frame][5:-4]),'is_falling'] = True
print(df)
df.to_csv(r"/Users/jerrychen/Desktop/PythonWorks/Fall_Detection/Feature_Extraction/features.csv", index=False)
